...
延时摄影生成模块使用FFmpeg高性能视频编码技术，将对齐后的图像序列转换为流畅的延时摄影视频。该模块支持多种视频质量和帧率选项，采用H.264编码确保最佳的压缩率和播放兼容性，能够生成适用于不同用途的专业级延时摄影作品。

## 🎯 主要功能

### ✨ 核心特性
- **多帧率支持**: 30fps、15fps、10fps三种预设帧率
- **H.264编码**: 使用libx264编码器确保高质量压缩
- **自适应质量**: 自动调整编码参数以平衡质量与文件大小
- **批量处理**: 一次性生成多个不同帧率的视频
- **进度监控**: 实时显示编码进度和状态
- **错误恢复**: 完善的异常处理和重试机制
- **内存优化**: 大图像序列的内存友好处理
...
### 🎬 视频特性
- **分辨率**: 保持原始图像分辨率
- **编码格式**: MP4容器 + H.264视频编码
- **颜色空间**: YUV420P（标准播放兼容性）
- **比特率**: 自适应质量控制
- **关键帧**: 优化的GOP结构
